...
public static class Common
{
    ...
    public static void _Redirect(string errorCode ,string ModuleID,string Appcation)
    {
        if (errorCode != "000")
            HttpContext.Current.Response.Redirect(string.Format("~/Admin/sMessage.aspx?error={0}&ModuleID={1}&Appcation={2}", errorCode,ModuleID,Appcation));
    }
    #endregion
    ...
    public static bool isCharDBC(char c)
    {
        if (c > 65280 && c < 65375)
            return true;
        else
    ...
    #region 循环获取checkboxlist的值
    public static string GetCheckValue(CheckBoxList CheckBox)
    {
        string s = "";
