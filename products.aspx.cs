...
public partial class products : System.Web.UI.Page
{
    ...
    public void bindDate(int _Page)
    {
        ...
        dt=dp.C_Pagination(new string[] { PageIndex, "20", "id,title,Img", "Prdct_info", " tag=0"+ type+ keywords, "ID DESC" }, ref RecordCount, ref PageCount);
        string _html = string.Empty;
        for (int i = 0; i < dt.Rows.Count; i++)
        {
            _html += "<div class=\"prol product\">";
            _html += "<div class=\"pic\"><a href=\"productinfo.aspx?id=" + dt.Rows[i]["id"].ToString() + "\" target=\"_blank\"></a><img src=\"UploadFile/images/" + dt.Rows[i]["Img"].ToString().Split('|')[0] + "\" alt=\"\"></div>";
            _html += "<div class=\"con\"><div class=\"tit\">" + dt.Rows[i]["title"].ToString() + "</div>";
            _html += "<div class=\"desc1\"></div>";
            _html += "<a href=\"productinfo.aspx?id=" + dt.Rows[i]["id"].ToString() + "\" class=\"more\" target=\"_blank\">More</a> </div></div>";
    ...
    private string goodstypelist()
    {
        ...
        if (dt.Rows.Count > 0)
        {
            ...
            {
                _html += "<li><a href=\"products.aspx?type="+ dt.Rows[i]["id"].ToString()+ "&txt=" + dt.Rows[i]["name"].ToString() + "\">" + dt.Rows[i]["name"].ToString() + "</a></li>";
